...
export function normalizeScriptSearchResult(
  ...
  return {
    ...
    songId: String(item.songId || item.song_id || '') || undefined,
    msgId: String(item.msgId || item.msg_id || '') || undefined,
    hash: item.hash || (source === 'kg' ? songmid : undefined),
    strMediaMid: String(item.strMediaMid || item.str_media_mid || item.mediaMid || item.media_mid || '') || undefined,
    copyrightId: String(item.copyrightId || item.copyright_id || item.songId || item.song_id || '') || undefined,
